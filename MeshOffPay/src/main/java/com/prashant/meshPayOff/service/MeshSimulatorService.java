 ...
 * devices that are nearby (we just say all devices are nearby for the simulation)
 * and copies packets between them, decrementing TTL each hop.
 *
 * When a device with internet (a "bridge node") holds a packet, the simulation's
...
public class MeshSimulatorService {
     ...
     * For the simulation we let everyone gossip with everyone in one round, which
